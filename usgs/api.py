...
def download_request(dataset, downloads, label, api_key=None):
    """
    ...
    payload = payloads.download_request(dataset, downloads, label)
...
def download_retrieve(label, api_key):
    api_key = _get_api_key(api_key)
    session = _create_session(api_key)

    url = '{}/download-retrieve'.format(USGS_API)
    payload = payloads.download_retrieve(label)
